...
var _default = {
    ...
    setCurrentGroupStyle: function setCurrentGroupStyle(evt) {
        ...
        var groupId = model.groupId, id = model.id;
        var customGroupControll = graph.get('customGroupControll');
        ...
        if (this.inGroupId && groupId) {
            ...
            var centerX = itemBBox.centerX, centerY = itemBBox.centerY;
            var minX = currentGroupBBox.minX, minY = currentGroupBBox.minY, maxX = currentGroupBBox.maxX, maxY = currentGroupBBox.maxY; // 在自己的group中拖动，判断是否拖出了自己的group
