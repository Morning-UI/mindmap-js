...
var _upperFirst = _interopRequireDefault(require("@antv/util/lib/upper-first"));
...
function () {
    ...
    ItemController.prototype.addItem = function (type, model) {
        ...
        var upperType = (0, _upperFirst.default)(type);
        var item = null; // 获取 this.get('styles') 中的值
        var styles = graph.get(type + (0, _upperFirst.default)(STATE_SUFFIX)) || {};
        ...
        if (mapper) {
            ...
            model = (0, _deepMix.default)({}, defaultModel, model, mappedModel);
        }
        else if (defaultModel) {
            // 很多布局会直接修改原数据模型，所以不能用 merge 的形式，逐个写入原 model 中
            (0, _each.default)(defaultModel, function (val, cfg) {
                if (!hasOwnProperty.call(model, cfg)) {
                    if ((0, _isObject.default)(val)) {
                        model[cfg] = (0, _clone.default)(val);
                    }
                    else {
        ...
        if (type === EDGE) {
            ...
            if (source && (0, _isString.default)(source)) {
                source = graph.findById(source);
            }
            if (target && (0, _isString.default)(target)) {
    ...
    ItemController.prototype.updateItem = function (item, cfg) {
        ...
        if (mapper) {
            var result = (0, _deepMix.default)({}, model, cfg);
            var mappedModel = mapper(result); // 将 update 时候用户传入的参数与mapperModel做deepMix，以便复用之前设置的参数值
            var newModel = (0, _deepMix.default)({}, model, mappedModel, cfg);
            if (mappedModel[STATE_SUFFIX]) {
                item.set('styles', newModel[STATE_SUFFIX]);
                delete newModel[STATE_SUFFIX];
            }
            (0, _each.default)(newModel, function (val, key) {
                cfg[key] = val;
            });
        }
        else {
            // merge update传进来的对象参数，model中没有的数据不做处理，对象和字符串值也不做处理，直接替换原来的
            (0, _each.default)(cfg, function (val, key) {
                if (model[key]) {
                    if ((0, _isObject.default)(val) && !(0, _isArray.default)(val)) {
    ...
    ItemController.prototype.setItemState = function (item, state, value) {
        ...
        if (item.hasState(stateName) === value || (0, _isString.default)(value) && item.hasState(stateName)) {
            return;
        }
